...
import { DEFAULT_LANGUAGE_SERVERS, getLanguageId } from "../../lsp/types.js";
...
export function registerLspCommands(program: Command): void {
  ...
  lspCmd
    .command("status")
    .description("Show status of active language servers")
    .option("-d, --directory <dir>", "Working directory", process.cwd())
    .option("--json", "Output as JSON")
    .action(async (options) => {
      const { LspManager } = await import("../../lsp/manager.js");
      ...
      if (options.json) {
        console.log(JSON.stringify({ activeServers }, null, 2));
        return;
      }

      if (activeServers.length === 0) {
        console.log(chalk.gray("No active language servers."));
        console.log(chalk.gray("\nLanguage servers are started on-demand when checking files."));
        return;
      }

      console.log(chalk.cyan("Active Language Servers:\n"));

      for (const languageId of activeServers) {
        console.log(`  ${chalk.white(languageId)}: ${chalk.green("running")}`);
      }
  ...
  lspCmd
    ...
    .option("--json", "Output as JSON")
    .action(async (options) => {
      const { LANGUAGE_IDS } = await import("../../lsp/types.js");
