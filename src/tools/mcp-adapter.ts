...
import type { McpTool, McpPrompt } from "../mcp/types.js";
...
export function registerMcpResourcesAndPrompts(manager: McpManager): void {
  ...
  if (resources.length > 0) {
    registry.register(createMcpListResourcesTool(manager));
    registry.register(createMcpResourceTool(manager));
  }
  
  // Register prompt tools
  const prompts = manager.getPrompts();
  if (prompts.length > 0) {
    registry.register(createMcpListPromptsTool(manager));
...
export function unregisterMcpResourcesAndPrompts(manager: McpManager): void {
  const resources = manager.getResources();
  if (resources.length > 0) {
    registry.unregister("mcp_list_resources");
    registry.unregister("mcp_read_resource");
  }
  
  const prompts = manager.getPrompts();
  if (prompts.length > 0) {
    registry.unregister("mcp_list_prompts");
    
    for (const prompt of prompts) {
      registry.unregister(prompt.name);
    }
  }
}
