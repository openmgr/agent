 ...
 */
export function getSkillPaths(workingDirectory: string, additionalBundledPaths: string[] = []): SkillPaths {
  return {
    local: join(workingDirectory, ".openmgr", "skills"),
    global: join(homedir(), ".config", "openmgr", "skills"),
    bundled: join(__dirname, "bundled"),
    additionalBundled: additionalBundledPaths,
  };
}
...
export class SkillManager {
  ...
  constructor(workingDirectory: string, options: SkillManagerOptions = {}) {
    this.workingDirectory = resolve(workingDirectory);
    this.paths = getSkillPaths(this.workingDirectory, options.additionalBundledPaths);
  }
  ...
  async discover(): Promise<SkillReference[]> {
    ...
    const sources: Array<{ path: string; source: SkillSource }> = [
      ...
      ...this.paths.additionalBundled.map(path => ({ path, source: "bundled" as SkillSource })),
      // Core bundled path
